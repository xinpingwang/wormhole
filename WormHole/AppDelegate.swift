...
class AppDelegate: NSObject, NSApplicationDelegate, NSUserNotificationCenterDelegate {
    ...
    func doToggleRunning(showToast: Bool) {
        ...
        if showToast {
            if isOn {
                self.makeToast("WormHole: On".localized)
            }
            else {
                self.makeToast("WormHole: Off".localized)
            }
        }
    }
    ...
    @IBAction func exportDiagnosis(_ sender: NSMenuItem) {
        ...
        savePanel.nameFieldStringValue = "WormHole_diagnose_\(dateString)"
    ...
    func updateMainMenu() {
        ...
        let isOn = defaults.bool(forKey: "ShadowsocksOn")
        if isOn {
            runningStatusMenuItem.title = "WormHole: On".localized
            runningStatusMenuItem.image = NSImage(named: "NSStatusAvailable")
            toggleRunningMenuItem.title = "Turn WormHole Off".localized
            let image = NSImage(named: "menu_icon")
            statusItem.image = image
        } else {
            runningStatusMenuItem.title = "WormHole: Off".localized
            toggleRunningMenuItem.title = "Turn WormHole On".localized
